...
(function(root) {
  var $fh = root.$fh || {};
  //v2 api will register the $fh.legacy_api flag, if not set, it means v2 apis are not loaded
  if (typeof $fh.legacy_api === "undefined") {
    //!!!lib start!!!
    ...
    var isAndroid = function() {
      var deviceInfo = window.device || navigator.device;
      var platform = deviceInfo.platform.toLowerCase();
    ...
    var __bind_ready = function() {
      ...
      if (isCordova()) {
        document.addEventListener("deviceready", __ready, false);
      } else {
